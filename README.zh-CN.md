...
- **产出的是成句，不是逐字流水账。** 每次输出都经过分层 compose：确定性口癖清理（嗯/呃 及口吃折叠）、受语境约束的纠错、按应用类别排版（列表与多主题分段），再在时限内采用本地 LLM 润色（含「这个/那个/就是」类口头语的语境化删除）。若模型丢失数字、URL、代码标识、已学习术语或改变原语言，保真护栏会拒绝该结果。
...
快捷键可在设置里改（提供 ⌥Space 备选）。翻译目标语言、上屏哪一侧、润色模型常驻内存（约 6GB，消除闲置后首句的模型加载等待）、开发者诊断也都在设置里。
