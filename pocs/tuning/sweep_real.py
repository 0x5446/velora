...
COMBOS = [
    {"id": "baseline", "args": []},
    {"id": "ship-greedy512", "args": ["-bs", "1", "-bo", "1", "-sns", "-ac", "AUTO"]},
    {"id": "beam2-512", "args": ["-bs", "2", "-bo", "2", "-sns", "-ac", "AUTO"]},
]
...
def resolve_args(args, duration):
    ...
    i = 0
    while i < len(args):
        if args[i] == "-ac" and args[i + 1] == "AUTO":
            out += ["-ac", str(min(1500, max(512, math.ceil(duration) * 50 + 64)))]
            i += 2
        else:
            out.append(args[i])
            i += 1
...
def main():
    ...
    (EVAL / f"real-sweep-{model_key}.json").write_text(json.dumps(results, ensure_ascii=False, indent=1))
    print("saved", EVAL / f"real-sweep-{model_key}.json")
