...
1. **qwen3:8b 拒绝删口癖，无论怎么写提示词**。7 组候选（含明确口癖词表、含演示删除的 few-shot、含"只允许删口癖"的白名单约束）全部保留"嗯/那个/um/uh"。这不是提示词工程能解决的问题，是该模型 + `format=json` + 低温下的行为特性。
   > **2026-07-14 更正**：此结论已被推翻。「删除规则 + 显式保留清单（指代/系词/顺序词/句尾语气词/叠词副词）+ 配套 few-shot」的组合能让 qwen3:8b 稳定删除语境填充词（这个/那个/就是/啊这些），门禁 `pocs/tuning/filler_eval.py` 11/11（含 6 个防误杀 guard）。当年 7 组候选缺的是保留清单——只给删除指令时模型宁可不动。规则层口癖清理仍然保留（发音类填充 嗯/呃 仍归规则层），两层分工见 `docs/LOCAL_MODEL_STRATEGY.md` 润色节。本报告其余部分保持当时原貌。
