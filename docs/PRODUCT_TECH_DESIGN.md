...
```text
输入模式：ASR（含纠错）→ compose { polished } → 保真护栏 → 上屏
翻译模式：ASR（含纠错）→ compose { polished, target } → 保真护栏 → 双语确认 → 单语上屏
```
...
- 翻译模式确认卡片默认同时展示原文和译文；确认后只插入用户选择的一侧，目标 App 内永远单语。
...
- 翻译模式：说语言 A，在确认卡片同时展示原文与语言 B 译文，用户确认后选择其中一侧单语上屏。
...
- 翻译模式固定双语审阅、单语上屏，校验能力留在确认卡片而不是污染目标文档。
...
当前场景格式路由：developer、work-chat、personal-chat、email、document、other。`app_format_profile` 只允许影响标点、大小写、分段和列表密度，禁止据此添加称呼、标题、问候、签名或改写事实与措辞。上下文取光标附近 AX 文本；拿不到光标范围时取字段末尾，避免长文档/终端误用最旧的开头文本。per-app 统计风格学习暂未启用。
...
- `polished` 保真校验：URL、数字、代码标识符、已学习术语不得丢失；无显式自纠正时整段编辑幅度不得异常。失败时丢弃 LLM 结果并回到规则地板。
- `target` opaque literal 校验：URL、数字和代码标识缺失时强制 review warning。

翻译插入策略：双语只存在于确认卡片；确认时用户选择原文或译文其中一侧，目标 App 中永远只插入单语。默认选择由设置中的首选插入语言决定。
...
| Inserted | 绿色短反馈约 1.2s | `presentationID` 隔离新旧展示；截止时直接 `NSPanel.orderOut`，切 Tab/Space 不影响消失 |
...
- 当前实现：录音期并行 context capture、hotword/history snapshot 与模型 prewarm；partial transcript speculative compose 仍是后续优化，不作为已上线能力。
...
- 输入模式直接上屏；翻译模式固定进入双语确认卡片，由用户选择单语上屏，不由模型置信度绕过。
...
录音期先按 App/附近文本/频次取 Top 12 热词；ASR 结束后再按本句拼音筛选要注入 compose 的 sound-alike（≤8）和 correction history（≤2）。这层二次门控避免把无关记忆整表塞入 prompt。
...
双语审阅数据由 `TranslationModeRenderer` 组装，不依赖翻译模型本身；真正的 `insert_text` 始终只取原文或译文一侧。
...
| Translation review usefulness | 双语确认卡片能让用户发现翻译问题，最终单语上屏不污染目标文档 |
...
| 本地 LLM 延迟高 | 润色慢 | 小模型、规则地板、deadline 回退；输入模式不因 LLM 阻塞，翻译模式固定确认 |
